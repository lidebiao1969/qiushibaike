		...
		<view v-if="commentList.length > 0" class="view-comment-list" v-for="(item, index) in commentList" :key="index">
			...
			<view class="view-comment-list-item">
				...
				<view class="comment-sub">
					...
					<view class="comment-sub-recomment" style="color: #999999;" v-if="item.CANDELETE == 1" @click="clickDelete(index)">删除</view>
					<view class="comment-sub-recomment" @click="clickRecomment(index)">回复</view>
				</view>
				<view class="comment-child" v-show="item.CHILD_ANWSER_LIST.length > 0">
					<view class="comment-child-item" v-for="(itemChild, indexChild) in item.CHILD_ANWSER_LIST" :key="indexChild">
						<text class="comment-child-item-name" @click="clickRecommentChild(index,indexChild)">{{ itemChild.FIRSTNICKNAME + '：' }}</text>
						...
						<text v-if="itemChild.CANDELETE == 1" class="comment-child-item-delete" @click="clickDeleteChild(index,indexChild)">删除</text>
					</view>
				</view>
			</view>
		</view>
...
export default {
	...
	methods: {
		 ...
		 * @param {Object} item
		 */
		clickRecomment(index,indexChild=0) {
			this.$emit('clickRecomment', index,indexChild);
		},
		/**
		 * 删除评论
		 * @param {Object} item
		 */
		clickDelete(index,indexChild=0) {
			this.$emit('clickDelete', index,indexChild);
		},
		/**
		 * 删除评论
		 * @param {Object} item
		 */
		clickDeleteChild(index,indexChild=0) {
			this.$emit('clickDeleteChild',index,indexChild);
		},
		/**
		 * 回复评论的评论
		 * @param {Object} item
		 */
		clickRecommentChild(index,indexChild=0) {
			this.$emit('clickRecommentChild', index,indexChild);
		}
	}
};
</script>
